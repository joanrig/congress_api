...
def search
  ...
  bills.each do |bill|
    ...
    if !Bill.find_by(bill_id: bill["bill_id"])
      Bill.create!(short_title: bill["short_title"]
      binding.pry
    end
  end
  ...
  # private
  #   def bill_params
  #     params.require(:bill).permit(:bill_id, :congress, :bill_type, :number, :title, :short_title, :sponsor_id, :gov_track_url, :introduced_date, :active, :last_vote, :house_passage, :senate_passage, :enacted, :vetoed, :cosponsors, :cosponsors )
